...
if st.button("Predict"):
    ...
    has_actual_data = formatted_date in df['Date'].dt.strftime("%d-%b-%Y").values
    ...
    if has_actual_data:
        ...
        diffs = {model: abs(pred - predicted_values[0]) for model, pred in predictions.items()}
    ...
    if has_actual_data:
        ...
        st.subheader("Differences (Relative Predicted):")
    ...
    if has_actual_data:
        ...
        for i in range(len(model_names)):
            ax.bar3d(x[i] + 0.5, y_predicted[i], 0, 0.4, 0.4, predicted_values[i], color=colors[i], alpha=0.8, label=f'Predicted ({model_names[i]})')
        ax.plot(x + 0.7, y_predicted + 0.2, predicted_values, color='blue', linestyle='-', linewidth=2)
        for i in range(len(model_names)):
            ax.text(x[i] + 0.7, 1.2, predicted_values[i],f'{predicted_values[i]:.2f}', ha='center', va='bottom', color='black')
    ...
    if has_actual_data:
        ...
        for i, name in enumerate(model_names):
            ax2.plot(name, predicted_values[i], marker='o', color=color_map[list(predictions.keys())[i]])
        ax2.set_ylabel('Close Price')
        ax2.set_title("Predicted Close Prices")
        ax2.legend()
